...
class VAE(nn.Module):
    def __init__(
        ...
        self.output_mean = None     # Will be computed after training
        self.output_std = None      # Will be computed after training
        self.squash_actions = squash_actions

        self.encoder = build_mlp(
            input_dim, hidden_dims, 2 * latent_dim,  # mu & logvar concatenated
    ...
    def sample(self, num_samples: int, device=None):
        """Sample from the VAE and apply output normalization if available."""
        if device is None:
        ...
        with torch.no_grad():
            ...
            clip_low = getattr(self, 'clip_low', None)
            clip_high = getattr(self, 'clip_high', None)

            if self.squash_actions:
                # Scale mode: squash to [-1,1], then optionally clamp again
                samples = torch.tanh(samples)
                if clip_low is not None and clip_high is not None:
                    if not torch.is_tensor(clip_low):
                        clip_low = torch.tensor(clip_low, device=device, dtype=samples.dtype)
                    if not torch.is_tensor(clip_high):
                        clip_high = torch.tensor(clip_high, device=device, dtype=samples.dtype)
                    samples = torch.clamp(samples, min=clip_low, max=clip_high)
            else:
                # Normalize mode: FIRST clip in RAW space if bounds provided, THEN z-score
                if clip_low is not None and clip_high is not None:
                    if not torch.is_tensor(clip_low):
                        clip_low = torch.tensor(clip_low, device=device, dtype=samples.dtype)
                    if not torch.is_tensor(clip_high):
                        clip_high = torch.tensor(clip_high, device=device, dtype=samples.dtype)
                    samples = torch.clamp(samples, min=clip_low, max=clip_high)
                # Now normalize using configured output stats (dataset stats by default)
                if self.output_mean is not None and self.output_std is not None:
                    output_mean = torch.tensor(self.output_mean, device=device, dtype=samples.dtype)
                    output_std = torch.tensor(self.output_std, device=device, dtype=samples.dtype)
                    samples = (samples - output_mean) / output_std
            
            return samples
    
    def compute_output_stats(self, num_mc_samples: int = 10000, device=None):
        """Compute output mean and std using Monte Carlo sampling."""
        if device is None:
            device = next(self.parameters()).device
        ...
        with torch.no_grad():
            ...
            z = torch.randn(num_mc_samples, self.latent_dim, device=device)
            samples = self.decoder(z)
            
            # Compute statistics
            self.output_mean = samples.mean(dim=0).cpu().numpy()
            self.output_std = samples.std(dim=0).cpu().numpy()
            # Avoid division by zero
            self.output_std = np.maximum(self.output_std, 1e-8)
            
        print(f"📊 VAE output stats computed from {num_mc_samples} samples:")
...
class NoiseToAction(nn.Module):
    ...
    def sample(self, num_samples: int, device=None):
        """Sample from the MLP and apply output normalization if available."""
        if device is None:
            device = next(self.parameters()).device
        
        with torch.no_grad():
            # Sample from random noise
            noise = torch.randn(num_samples, self.input_dim, device=device)
            samples = self.net(noise)
            # Optional clipping bounds
            clip_low = getattr(self, 'clip_low', None)
            clip_high = getattr(self, 'clip_high', None)
    ...
    def compute_output_stats(self, num_mc_samples: int = 10000, device=None):
        ...
        with torch.no_grad():
            # Generate many samples
            noise = torch.randn(num_mc_samples, self.input_dim, device=device)
            samples = self.net(noise)
        ...
        print(f"📊 MLP output stats computed from {num_mc_samples} samples:")
        print(f"📊 Output mean: {self.output_mean}")
        print(f"📊 Output std:  {self.output_std}")
...
def main():
    args = parse_args()
    action_dim_by_task = {"pendulum": 1, "walker2d": 6}
    obs_dim_by_task = {"pendulum": 3, "walker2d": 17}

    action_dim = action_dim_by_task[args.task]
    obs_dim = obs_dim_by_task[args.task]
